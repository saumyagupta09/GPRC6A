...
def read_sequential_phy(filepath):
    """
    Reads a PHYLIP-like sequential file:
      header: 'ntaxa nsites'
      then for each taxon: one line of name, followed by wrapped sequence lines
    ...
    def is_seq_line(s: str) -> bool:
        s = s.strip()
        return len(s) > 0 and set(s) <= IUPAC_VALID
    ...
    for idx, raw in enumerate(lines[1:], start=2):
        ...
        if is_seq_line(s):
            if cur is None:
                raise ValueError(f"Sequence line before a taxon name at line {idx}.")
    ...
    if nsites_hdr != nsites:
        print(f"Warning: header nsites={nsites_hdr}, parsed {nsites} sites.", file=sys.stderr)
    ...
    if wrap_len is None or wrap_len <= 0:
        wrap_len = 60  # fallback

    return taxa, seqs, nsites, wrap_len
