...
#5. MARK PUTATIVE EXON BOUNDARIES
input_file="${sps}_absolute_lengths.txt"
output_file="new_${sps}_absolute_lengths.txt"

# Read the first line to check if it contains 4000 or 7000 in the length column
read first_line < "$input_file"
first_len=$(echo "$first_line" | awk '{print $4}')
last_line=$(tail -n 1 "$input_file")


start=0

# Function to write putative exon and intron lines
write_putative_exon_intron() {
    local exon_name="$1"
    local intron_name="$2"
    local exon_len="$3"
    local intron_len="$4"
    local exon_end=$(expr $start + $exon_len)
    local intron_start=$exon_end
    local intron_end=$(expr $intron_start + $intron_len)

    echo -e "$exon_name\t$start\t$exon_end\t$exon_len" >> "$output_file"
    echo -e "$intron_name\t$intron_start\t$intron_end\t$intron_len" >> "$output_file"
    
    start=$intron_end  # Update start for the next line
}

# Process the first line if it has 4000 or 7000 as the length value
if [[ "$first_len" == "4000" ]]; then
    write_putative_exon_intron "putative exon_1" "intron_1" 1000 3000
elif [[ "$first_len" == "7000" ]]; then
    write_putative_exon_intron "putative exon_1" "intron_1" 1000 3000
    write_putative_exon_intron "putative exon_2" "intron_2" 1000 2000
else
    # No modification for the first line if it doesn't contain 4000 or 7000
    echo "$first_line" > "$output_file"
    start=$(echo "$first_line" | awk '{print $3}')  # Update start based on first line
fi

# Process the remaining lines (excluding the last line)
tail -n +2 "$input_file" | head -n -1 | while read line; do
    name=$(echo $line | awk '{print $1}')
    len=$(echo $line | awk '{print $4}')
    end=$(expr $start + $len)
    
    echo -e "$name\t$start\t$end\t$len" >> "$output_file"
    
    start=$end  # Update start for the next iteration
done

# Special handling for the last line if it contains "exon_6_downstream1kb"
if [[ "$last_line" == *"exon_6_downstream1kb"* ]]; then
    intron_name="intron_5"
    intron_len=2000
    putative_exon_name="putative exon_6"
    putative_exon_len=2000

    intron_start=$start
    intron_end=$(expr $intron_start + $intron_len)
    
    exon_start=$intron_end
    exon_end=$(expr $exon_start + $putative_exon_len)

    echo -e "$intron_name\t$intron_start\t$intron_end\t$intron_len" >> "$output_file"
    echo -e "$putative_exon_name\t$exon_start\t$exon_end\t$putative_exon_len" >> "$output_file"
else
    # If last line doesn't match, just append it as it is
