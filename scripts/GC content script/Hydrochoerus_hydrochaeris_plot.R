...
input_file <- "Hydrochoerus_hydrochaeris_gcpcnt.bed"  # Update with actual path if needed
rectangles_file <- "new_Hydrochoerus_hydrochaeris_absolute_lengths.txt"  # Update with actual path if needed
...
p <- ggplot() +
  ...
  # Set the Y-axis breaks in increments of 0.05, limits set to 0.90
  scale_y_continuous(breaks = seq(0, 0.90, by = 0.05), limits = c(0, 0.90)) +
  labs(x = "Position (bp)", y = "GC content") +
  ggtitle(expression(italic("Hydrochoerus hydrochaeris"))) +
  theme_minimal() +
...
ggsave("Hydrochoerus_hydrochaeris_gccontent_plot.pdf", plot = p, width = 10, height = 6)
