...
def main() -> int:
    ...
    if len(bg_species) < 3:
        raise RuntimeError(
            "Fewer than 3 defensible intact background species are available after tiered selection. "
            f"Inspect {bg_diag} for the exact status/callable reason for every species. "
            "Do not force PAML with an inadequate background."
        )
